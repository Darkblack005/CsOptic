...
Trade.prototype.startBots = function startBots(done) {
    ...
    async.eachOfSeries(config.bots, (bot, id, callback) => {
        ...
        self.instances[id].community.on('sessionExpired', (err) => {
        ...
        if (offer.state == TradeOfferManager.ETradeOfferState.Accepted) {
            offer.getExchangeDetails((err, status, tradeInitTime, receivedItems, sentItems) => {
                ...
                if(!oldSentItems.length > 0) {
                    // We're making or joining a flip
                    oldReceivedItems.forEach(function(e, index) {
                        self.assetIdMap.set(e, newReceivedItems[index])
                    })
                    newReceivedItems.forEach(function(e, index) {
                        self.flipmanager.winningsOwed[i].items.push(e)
                    })
                    ...
                    // We've sent the items back and the user accepted the trade
                    // So delete them from the winningsOwed array
                    for(var i = 0; i < self.flipmanager.winningsOwed.length; i++) {
                        if(self.flipmanager.winningsOwed[i].steamID64 == sid64) {
                            oldSentItems.forEach(function(e) {
                                self.flipmanager.winningsOwed[i].items.splice(self.flipmanager.winningsOwed[i].items.indexOf(e), 1)
                            })
                        }
                    }
                }
