...
FlipManager.prototype.sendWinner = function sendWinner(ctWin, coinflipIndex) {
    ...
    self.Trade.getInventory('76561198159926602', '730', '2', (err, data) => {
        ...
        async.forEach(flipItems, (index, cb) => {
            ...
            var found = false
            for(var i = 0; i < self.flipmanager.winningsOwed.length && !found; i++) {
                if(self.flipmanager.winningsOwed[i].steamID64 == winnerSID64) {
                    self.flipmanager.winningsOwed[i].items.push(flipItems)
                    found = true
                }
            }

            if(!found) {
                self.flipmanager.winningsOwed.push({
                    steamID64: winnerSID64,
                    items: newReceivedItems
                })
            }
