...
FlipManager.prototype.createNewFlip = function createNewFlip(data, itemsAndDetails) {
    ...
    var foundUndefined = null;
    for(var i = 0; i < this.currentflips.length; i++) {
        if(!this.currentflips[i]) {
            foundUndefined = i;
            break;
        }
    }

    // Since we're just delete'ing flips when theyre over we have to find where they were deleted and fill in those 'undefined's
    if(!foundUndefined) {
        ...
        this.currentflips[foundUndefined] = {
            flipDetails: data,
            itemsAndDetails: itemsAndDetails,
            serverSeed: serverSeed,
            finalHash: hash,
