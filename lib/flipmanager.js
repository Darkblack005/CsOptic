...
FlipManager.prototype.evaluateFlip = function evaluateFlip(steamID64) {
    ...
    for(var i = 0; i < this.currentJoined.length; i++) {
        if(this.currentJoined[i].data.steamID64 == steamID64) {
            ...
            this.sendWinner(parseInt(this.currentflips[i].serverSeed+this.currentflips[i].flipDetails.clientSeed, 16) % 100 > 49)
            break
        }
    }
}
...
// T 0-49
// CT 50-99
FlipManager.prototype.sendWinner = function sendWinner(ctWin) {
    console.log('did the ct win?')
    console.log(ctWin)
}
