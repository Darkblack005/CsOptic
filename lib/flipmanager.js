...
FlipManager.prototype.tradeAccepted = function tradeAccepted(steamID64) {
    ...
    for(j = 0; j < this.currentPendingJoiners; j++) {
        if(this.currentPendingJoiners[i].steamID64 === steamID64) {
            isAJoiner = true
            break
        }
    }
    ...
    if(!isAJoiner) {
        ...
        this.currentJoined[j].push(this.currentPendingJoiners[j])
...
FlipManager.prototype.changeFlipJoinableByFlipIndex = function changeFlipJoinableByFlipIndex(index, newVal) {
    this.currentflips[index].joinable = newVal
    this.currentFlipsDataForPublic[index].joinable = newVal
}

FlipManager.prototype.joinFlip = function joinFlip(data, itemsAndDetails) {
    this.currentPendingJoiners.push({
        data: data,
        itemsAndDetails: itemsAndDetails
    })
}
