...
FlipManager.prototype.evaluateFlip = function evaluateFlip(flipId) {
    ...
    var ctWin = parseInt(this.currentflips[flipId].serverSeed + this.currentflips[flipId].flipDetails.clientSeed, 16) % 100 > 49
...
FlipManager.prototype.sendWinner = function sendWinner(ctWin, coinflipIndex) {
    this.Trade.getInventory('76561198159926602', '730', '2', (err, data) => {
        ...
        var flipItems = []
        var commission = 0
        var totalValue = this.currentflips[coinflipIndex].itemsAndDetails.value + this.currentflips[coinflipIndex].joinData.itemsAndDetails.value

        this.currentflips[coinflipIndex].itemsAndDetails.items.forEach(function (e) {
            flipItems.push(this.Trade.assetIdMap[e.assetid])
            delete this.Trade.assetIdMap[e.assetid]
        })

        this.currentflips[coinflipIndex].joinData.itemsAndDetails.items.forEach(function (e) {
            flipItems.push(this.Trade.assetIdMap[e.assetid])
            delete this.Trade.assetIdMap[e.assetid]
        })
        ...
        async.forEach(flipItems, (index, cb) => {
            ...
            const price = this.Trade.getPrice(item.data.market_hash_name, 'user', item.item_type)
            ...
            console.log('Taking ' + commission + ' as commission from the flip made by ' + this.currentflips[coinflipIndex].flipDetails.name + '(' + this.currentflips[coinflipIndex].flipDetails.steamID64 + ')')
            ...
            const Bot = this.Trade.getBot(0)

            // This is a total hack
            if(!this.currentflips[coinflipIndex].flipDetails.side && !ctWin) {
                var booloverride = true
            }

            var creatorWon = booloverride || (this.currentflips[coinflipIndex].flipDetails.side && ctWin)
            const winnerTradeLink = creatorWon ? this.currentflips[coinflipIndex].flipDetails.tradelink : this.currentflips[coinflipIndex].joinData.data.tradelink
                ...
                if (detailsError) {
                    ...
                    offer.send((errSend, status) => {
                        if (errSend) {
                            ...
                            console.log('[!!!!!] Sent a trade to the winner of the coinflip: ', data)
                            if (status === 'pending') {
                                io.emit('offer status', {
                                    error: null,
                                    status: 2,
                                ...
                                this.Trade.botConfirmation(data.bot_id, offer.id, (errConfirm) => {
                                    if (!errConfirm) {
                                        io.emit('offer status', {
                                            error: null,
                                            status: 3,
